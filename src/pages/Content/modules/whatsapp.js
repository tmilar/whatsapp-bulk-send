export default () => {
  ...
  const getElementsByXpath = path => _xPathResultsToArray(document.evaluate(path, document, null, XPathResult.ANY_TYPE, null))

  // parser
  const getSubmitMsgButton = () => getOneElementByXpath('//*[@id="main"]/footer/div[1]/div[3]/button')

  const getLastSentMessage = () => {
    const sentMessages = getElementsByXpath('//*[@id="main"]//div[contains(@class, \'message-out\')]//span[contains(@class, \'selectable-text\')]/span')
    if (!sentMessages || !sentMessages.length) {
      return null
    }
    const { innerText: message } = sentMessages.pop()

    const sentMessagesStatuses = getElementsByXpath('//*[@id="main"]//div[contains(@class, \'message-out\')]/div[1]/div[1]/div[1]/div[2]//span[@data-icon]')
